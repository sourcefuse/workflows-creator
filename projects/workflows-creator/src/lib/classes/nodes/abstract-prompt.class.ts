...
/**
 * This is the WorkflowPrompt class. WorkflowPrompts are used to prompt the user
 * for a value to be used in the workflow. They are used in the WorkflowNode class.
 */
export abstract class WorkflowPrompt {
  abstract suffix?: string | {state: string};
  abstract prefix?: string | {state: string};
  abstract typeFunction: <S extends RecordOfAnyType>(
    state: State<S>,
  ) => InputTypes;
